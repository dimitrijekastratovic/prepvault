...
from app.submissions.exceptions import ProblemNotFound, SubmissionConflict
...
def get_submission(user_id: int, idempotency_key: str, session: Session) -> Submission | None:
    return session.exec(
        select(Submission)
...
async def create_submission(user: User, 
    ...
    if idempotency_key:
        existing_submission = get_submission(user.id, idempotency_key, session)
