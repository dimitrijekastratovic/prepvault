...
@pytest.fixture
def test_submission(user_id, problem_id):
    return {
        "user_id": user_id,
...
@pytest.fixture
def submission_id(session, test_submission) -> int:
    return add_test_submission(
        session=session,
        user_id=test_submission["user_id"],
        problem_id=test_submission["problem_id"],
        code=test_submission["code"],
        language=test_submission["language"],
        status=test_submission["status"],
        judge0_token=test_submission["judge0_token"],
        stdout=test_submission["stdout"],
        stderr=test_submission["stderr"],
        compile_output=test_submission["compile_output"],
        runtime_ms=test_submission["runtime_ms"],
        memory_kb=test_submission["memory_kb"],
        idempotency_key=test_submission["idempotency_key"],
    )
